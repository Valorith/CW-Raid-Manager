...
import {
  createOrReplaceBisBan,
  ensureBisAdmin,
  getBisBoard,
  listActiveBisBans,
  nominateBisCandidate,
  revokeBisBan,
  searchBisCharactersByName,
  searchBisUsers,
  searchCompatibleDiscoveredItems,
  voteForBisCandidate
} from '../services/bisService.js';
...
async function requireBisAdmin(
...
export async function bisRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get('/discovered-items', { preHandler: [authenticate] }, async (request, reply) => {
    ...
    const items = await searchCompatibleDiscoveredItems(
  ...
  server.get(
    '/admin/users/search',
    { preHandler: [authenticate, requireBisAdmin] },
    async (request, reply) => {
      const querySchema = z.object({
        q: z.string().trim().min(2).max(100)
      });
