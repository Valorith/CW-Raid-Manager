...
export async function availabilityRoutes(server: FastifyInstance): Promise<void> {
  /**
  ...
  server.put(
    ...
    async (request, reply) => {
      ...
      try {
        const entries = await updateUserAvailability(
          request.user.userId,
          guildId,
          parsed.data.updates
        );
  ...
  server.delete(
    ...
    async (request, reply) => {
      ...
      try {
        const deleted = await deleteUserAvailability(
          request.user.userId,
          guildId,
          parsed.data.dates
        );
