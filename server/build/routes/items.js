...
export async function itemRoutes(server) {
    /**
    ...
    server.get('/items/:itemId/stats', async (request, reply) => {
        ...
        if (item.proceffect > 0)
            spellIds.push(item.proceffect);
        if (item.worneffect > 0)
            spellIds.push(item.worneffect);
        if (item.clickeffect > 0)
            spellIds.push(item.clickeffect);
        if (item.focuseffect > 0)
            spellIds.push(item.focuseffect);
        if (item.scrolleffect > 0)
            spellIds.push(item.scrolleffect);
        if (item.bardeffect > 0)
            spellIds.push(item.bardeffect);
    ...
    server.post('/items/stats/batch', async (request, reply) => {
        ...
        for (const item of itemsMap.values()) {
            if (item.proceffect > 0)
                spellIds.add(item.proceffect);
            if (item.worneffect > 0)
                spellIds.add(item.worneffect);
            if (item.clickeffect > 0)
                spellIds.add(item.clickeffect);
            if (item.focuseffect > 0)
                spellIds.add(item.focuseffect);
            if (item.scrolleffect > 0)
                spellIds.add(item.scrolleffect);
            if (item.bardeffect > 0)
                spellIds.add(item.bardeffect);
        }
        // Fetch spell names
        let spellNames = {};
        if (spellIds.size > 0) {
