...
import { ensureMarketListingsFresh, getMarketListingsPage } from '../services/marketListingsService.js';
import { addMarketFavoriteCharacter, addMarketFavoriteItem, getMarketFavorites, getMarketCharacterHistoryPage, getMarketItemActivity, getMarketItemHistory, getMarketRecentSalesPage, getMarketSummary, removeMarketFavoriteCharacter, removeMarketFavoriteItem, searchMarketCharacters, searchMarketItems } from '../services/marketService.js';
import { searchDiscoveredItems } from '../services/npcRespawnService.js';
export async function marketRoutes(server) {
    const itemTypeValues = new Set(EQEMU_ITEM_TYPE_VALUES);
    const itemSlotIds = new Set(EQEMU_ITEM_SLOT_IDS);
    server.get('/listings', {
        ...
        const querySchema = z
            ...
            dealsOnly: z
                .preprocess((value) => value === true || value === 'true'
                ? true
            ...
            .refine((value) => value.minCharges == null ||
        ...
        const refreshIfStale = typeof request.query === 'object' &&
            request.query !== null &&
    ...
    server.post('/favorites/characters', {
        preHandler: [authenticate]
    }, async (request, reply) => {
        const bodySchema = z.object({
        ...
        try {
            const favorite = await addMarketFavoriteCharacter(request.user.userId, parsed.data.characterName);
            return { favorite };
        }
        catch (error) {
    ...
    server.delete('/favorites/characters', {
        preHandler: [authenticate]
    }, async (request, reply) => {
    ...
    server.get('/items/discovered-search', {
        preHandler: [authenticate]
    }, async (request, reply) => {
        const querySchema = z.object({
