...
import { getActiveLootMonitorSession, heartbeatLootMonitorSession, LOOT_MONITOR_HEARTBEAT_INTERVAL_MS, LOOT_MONITOR_SESSION_TTL_MS, startLootMonitorSession, stopLootMonitorSession, getLootCouncilState, updateLootCouncilState } from '../services/logMonitorService.js';
...
export async function lootRoutes(server) {
    ...
    server.post('/raids/:raidId/loot-council-state', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        updateLootCouncilState(raidId, parsed.data.items);
        return { success: true };
    });
