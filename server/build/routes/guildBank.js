...
import { addGuildBankCharacter, fetchGuildBankSnapshot, listGuildBankCharacters, removeGuildBankCharacter } from '../services/guildBankService.js';
...
export async function guildBankRoutes(server) {
    server.get('/:guildId/guild-bank', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        try {
            ...
            if (error instanceof Error &&
                ...
                return reply
                    .serviceUnavailable('EQ database is not configured; set EQ_DB_* environment variables to enable guild bank lookups.');
            }
    ...
    server.post('/:guildId/guild-bank/request-items', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        for (const item of snapshot.items) {
            const quantity = typeof item.charges === 'number' && Number.isFinite(item.charges) && item.charges > 0
                ? item.charges > 1000
                    ? 1
                    : item.charges
                : 1;
            ...
            if (!availableByKey.has(key)) {
                availableByKey.set(key, { total: 0, entries: [] });
            }
        ...
        for (const requestItem of parsedBody.data.items) {
            const key = requestItem.itemId != null
                ? `id:${requestItem.itemId}`
                : `name:${requestItem.itemName.toLowerCase()}`;
            const bucket = availableByKey.get(key);
            if (!bucket || bucket.total < requestItem.quantity) {
                return reply.badRequest(`Insufficient quantity for ${requestItem.itemName} (requested ${requestItem.quantity}, available ${bucket?.total ?? 0}).`);
            }
            ...
            allocations.push({
                ...
                itemIconId: bucket.entries.find((entry) => entry.itemIconId != null)?.itemIconId ?? null,
