...
export async function guildRoutes(server) {
    ...
    server.put('/:guildId/webhooks/:webhookId', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        const sanitizedWebhookUrl = parsed.data.webhookUrl === undefined ? undefined : parsed.data.webhookUrl?.trim() ?? '';
        ...
        try {
            const webhook = await updateGuildDiscordWebhook(webhookId, guildId, {
                ...parsed.data,
                webhookUrl: sanitizedWebhookUrl === undefined
                    ? undefined
                    : sanitizedWebhookUrl.length > 0
                        ? sanitizedWebhookUrl
                        : null,
                usernameOverride: parsed.data.usernameOverride === undefined
                    ? undefined
                    : (parsed.data.usernameOverride ?? '').trim() || null,
                avatarUrl: parsed.data.avatarUrl === undefined
                    ? undefined
                    : (parsed.data.avatarUrl ?? '').trim() || null,
                mentionRoleId: parsed.data.mentionRoleId === undefined
                    ? undefined
    ...
    server.post('/:guildId/applications/:applicationId/approve', { preHandler: [authenticate] }, async (request, reply) => {
        const paramsSchema = z.object({
            guildId: z.string(),
    ...
    server.post('/:guildId/applications/:applicationId/deny', { preHandler: [authenticate] }, async (request, reply) => {
        const paramsSchema = z.object({
            guildId: z.string(),
            applicationId: z.string()
        });
