...
import { fetchPlayerEventLogs } from './playerEventLogsService.js';
...
function mapEqRaceIdToName(raceId) {
    switch (raceId) {
        case 1: return 'Human';
...
function mapGenderIdToName(genderId) {
    switch (genderId) {
        case 0: return 'Male';
...
async function checkCharacterCorpsesTable() {
...
export async function getCharacterByName(characterName) {
  ...
  `;
    ...
    LIMIT 1
  `;
    const rows = await queryEqDb(query, [characterName]);
    if (rows.length === 0) {
        return null;
    }
    const row = rows[0];
    ...
    let guildName = null;
    try {
        const guildRows = await queryEqDb(`
...
export async function getCharacterById(characterId) {
  ...
  `;
    ...
    LIMIT 1
  `;
    const rows = await queryEqDb(query, [characterId]);
    if (rows.length === 0) {
        return null;
    }
    const row = rows[0];
 ...
 */
export async function getCharacterEvents(characterId, filters = {}) {
...
export async function getAccountInfo(accountId) {
    ...
    if (accountColumns.includes('charname'))
        selectFields += ', a.charname';
    if (accountColumns.includes('status'))
        selectFields += ', a.status';
    if (accountColumns.includes('lsaccount_id'))
        selectFields += ', a.lsaccount_id as ls_account_id';
    if (accountColumns.includes('gmspeed'))
        selectFields += ', a.gmspeed as gm_speed';
    if (accountColumns.includes('hideme'))
        selectFields += ', a.hideme';
    if (accountColumns.includes('suspend_reason'))
        selectFields += ', a.suspend_reason';
    if (accountColumns.includes('suspendeduntil'))
        selectFields += ', a.suspendeduntil as suspended_until';
    if (accountColumns.includes('time_creation'))
        selectFields += ', a.time_creation';
    if (accountColumns.includes('ip_exemption_multiplier'))
        selectFields += ', a.ip_exemption_multiplier as ip_exemptions';
    if (accountColumns.includes('created_at'))
        selectFields += ', a.created_at';
    if (accountColumns.includes('updated_at'))
        selectFields += ', a.updated_at';
    const query = `SELECT ${selectFields} FROM account a WHERE a.${accountSchema.idColumn} = ?`;
    const rows = await queryEqDb(query, [accountId]);
    if (rows.length === 0) {
        return null;
    }
    const row = rows[0];
    ...
    if (charSchema) {
        const countRows = await queryEqDb(`SELECT COUNT(*) as count FROM character_data WHERE ${charSchema.accountIdColumn} = ?`, [accountId]);
 ...
 */
export async function getCharacterCorpses(characterId) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured.');
    }
  ...
  `;
    ...
    if (corpseIds.length > 0) {
        try {
            const itemCountRows = await queryEqDb(`
 ...
 */
export async function getCharacterAssociates(characterId) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured.');
    }
    ...
    const charRows = await queryEqDb(`SELECT ${charSchema.accountIdColumn} as account_id FROM character_data WHERE ${charSchema.idColumn} = ?`, [characterId]);
    ...
    const sameAccountChars = await queryEqDb(`
    SELECT
      cd.${charSchema.idColumn} as id,
      cd.${charSchema.nameColumn} as name,
      cd.${charSchema.accountIdColumn} as account_id,
  ...
  `, [accountId, characterId]);
    ...
    const sameIpAccountIds = new Set(); // Track accounts found via same IP
    try {
        // First, get all IPs this character has connected from (from connections table)
        const ipRows = await queryEqDb(`
      SELECT DISTINCT ip
      FROM connections
      WHERE characterid = ?
    `, [characterId]);
        ...
        if (ips.length > 0) {
            ...
            const sameIpChars = await queryEqDb(`
        SELECT DISTINCT
          c.characterid as id,
          cd.${charSchema.nameColumn} as name,
          cd.${charSchema.accountIdColumn} as account_id,
    ...
    if (sameIpAccountIds.size > 0) {
        try {
            ...
            const allAccountChars = await queryEqDb(`
        SELECT
          cd.${charSchema.idColumn} as id,
          cd.${charSchema.nameColumn} as name,
          cd.${charSchema.accountIdColumn} as account_id,
    ...
    for (const assoc of manualAssociations) {
        ...
        try {
            const charDetails = await queryEqDb(`
        SELECT
          cd.${charSchema.idColumn} as id,
          cd.${charSchema.nameColumn} as name,
          cd.${charSchema.accountIdColumn} as account_id,
      ...
      `, [otherCharId]);
            if (charDetails.length > 0) {
                ...
                associates.push({
                    ...
                    manualAssociationType: isAuto ? undefined : assoc.associationType,
 ...
 */
export async function searchCharacters(searchTerm, limit = 20) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured.');
    }
 ...
 */
export async function addManualAssociation(sourceCharacterId, targetCharacterId, reason, associationType, userId, userName) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured.');
    }
    const charSchema = await discoverCharacterDataSchema();
 ...
 */
export async function removeManualAssociation(associationId) {
 ...
 */
export async function createAccountNote(accountId, content, userId, userName) {
 ...
 * Update a note
 */
export async function updateAccountNote(noteId, content) {
 ...
 * Delete a note
 */
export async function deleteAccountNote(noteId) {
 ...
 */
export async function syncIpGroupAssociations(connections, userId, userName) {
    ...
    async function createAssociation(charA, charB, ip) {
    ...
    try {
        ...
        const tables = await queryEqDb(`SHOW TABLES LIKE 'account_ip'`);
        if (tables.length === 0) {
            return { created, skipped };
        }
        ...
        for (const ip of uniqueIps) {
            ...
            const historicalAccounts = await queryEqDb(`SELECT accid as accountId FROM account_ip WHERE ip = ?`, [ip]);
            ...
            const otherChars = await queryEqDb(`
        SELECT
          ${charSchema.idColumn} as id,
          ${charSchema.nameColumn} as name,
      ...
      `, otherAccountIds);
            ...
            for (const conn of currentConns) {
                for (const otherChar of otherChars) {
                    if (await createAssociation(conn, { characterId: otherChar.id, characterName: otherChar.name, accountId: otherChar.accountId }, ip)) {
                        created++;
                    }
                    else {
                        skipped++;
 ...
 */
export async function getAccountKnownIps(accountId) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured.');
    }
    try {
        ...
        const rows = await queryEqDb(`SELECT ${selectFields} FROM account_ip WHERE accid = ? ORDER BY ${orderBy}`, [accountId]);
        return rows.map(row => {
            let lastUsed = null;
            if (row.lastUsed) {
...
export async function getAccountsByIp(ip) {
    ...
    try {
        ...
        const rows = await queryEqDb(`SELECT accid as accountId, count FROM account_ip WHERE ip = ?`, [ip]);
        return rows.map(row => ({
 ...
 */
export async function autoLinkSharedIps(userId, userName) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured.');
    }
    let created = 0;
    ...
    try {
        ...
        const sharedIpData = await queryEqDb(`
    ...
    `);
        ...
        const allCharacters = await queryEqDb(`
      SELECT
        ${charSchema.idColumn} as id,
        ${charSchema.nameColumn} as name,
        ${charSchema.accountIdColumn} as accountId
 ...
 * Uses bulk operations for efficiency.
 */
export async function* autoLinkSharedIpsStream(userId, userName) {
    if (!isEqDbConfigured()) {
