 ...
 */
async function isWebhookProcessingEnabled() {
    try {
 ...
 */
export async function getWebhookProcessingEnabled() {
    return isWebhookProcessingEnabled();
}
/**
 * Get detailed webhook processing status for this server.
 */
export async function getWebhookProcessingStatus() {
 ...
 */
export async function setWebhookProcessingEnabled(enabled) {
...
export function normalizeRetentionPolicy(input) {
    ...
    if (input.mode === 'maxCount') {
        const maxCount = typeof input.maxCount === 'number' && input.maxCount > 0 ? Math.floor(input.maxCount) : 5000;
        return { mode: 'maxCount', maxCount };
    }
    return { mode: 'indefinite' };
}
...
export async function createInboundWebhook(userId, input) {
...
export async function updateInboundWebhook(webhookId, input) {
    const data = {};
...
export async function deleteInboundWebhook(webhookId) {
    await prisma.inboundWebhook.delete({
        where: { id: webhookId }
    });
}
export async function createInboundWebhookAction(webhookId, input) {
...
export async function updateInboundWebhookAction(actionId, input) {
    const data = {};
...
export async function deleteInboundWebhookAction(actionId) {
...
export async function getInboundWebhookMessage(messageId, userId) {
...
export async function retryCrashReviewForMessage(messageId) {
    const message = await prisma.inboundWebhookMessage.findUnique({
        where: { id: messageId }
    });
    if (!message) {
    ...
    if (message.payload && typeof message.payload === 'object' && !Array.isArray(message.payload)) {
        const payloadCopy = { ...message.payload };
    ...
    try {
        ...
        const findingsRecord = findings;
        const signature = findingsRecord?.signature;
        const summaryText = typeof findingsRecord?.summary === 'string' ? findingsRecord.summary : '';
        ...
        const discordAction = webhook.actions.find((action) => action.type === 'DISCORD_RELAY' && action.isEnabled);
        if (discordAction) {
        ...
        const clawdbotAction = webhook.actions.find((action) => action.type === 'CLAWDBOT_RELAY' && action.isEnabled);
        if (clawdbotAction) {
...
export async function receiveInboundWebhookMessage(input) {
    ...
    if (!processingEnabled) {
        ...
        return { id: 'skipped', webhookId: input.webhookId, status: 'SKIPPED' };
    }
    const webhook = await prisma.inboundWebhook.findUnique({
    ...
    const payloadKeys = input.payload && typeof input.payload === 'object' ? Object.keys(input.payload) : [];
    ...
    if (!messageContent || messageContent.trim().length === 0) {
        ...
        return { id: 'discarded', webhookId: webhook.id, status: 'DISCARDED' };
    }
...
async function runInboundWebhookActions(messageId, actions, payload, webhookLabel, devMode = false) {
    let overallStatus = 'PROCESSED';
    const summary = [];
    let payloadForActions = payload;
...
async function updateActionSummary(messageId, actionId, status) {
    const message = await prisma.inboundWebhookMessage.findUnique({
        where: { id: messageId },
...
async function reviewCrashReportWithRetry(input, config) {
    let attempt = 0;
    let lastError;
    while (attempt < 2) {
        attempt += 1;
        try {
            const findings = await withTimeout(reviewCrashReport(input, {
...
function buildCrashReviewInput(payload, rawBody, fallbackPayload) {
 ...
 */
function looksLikeCrashReport(text) {
...
function extractCrashReportSections(text) {
    const lines = text.split(/\r?\n/);
    let exceptionLine = '';
    let symInitLine = '';
    let osVersionLine = '';
...
function enrichPayloadWithCrashReview(payload, findings, attempts, errorType) {
...
function normalizeActionConfig(config) {
    ...
    return {
        ...
        crashMaxOutputTokens: typeof raw.crashMaxOutputTokens === 'number' ? raw.crashMaxOutputTokens : undefined,
 ...
 */
function getDiscordUrl(config, devMode) {
 ...
 */
function getClawdbotUrl(config, devMode) {
 ...
 */
async function sendClawdbotRelay(url, payload, config, rawBody) {
...
function buildClawdbotPayload(payload, config, rawBody) {
...
async function sendDiscordRelay(url, payload, config, messageId) {
...
function buildDiscordPayload(payload, config, messageId) {
...
function formatDiscordContent(payload, template) {
    const raw = safeJson(payload);
    if (!raw) {
...
function buildCrashReviewEmbed(findings, _template, messageId) {
...
function detectQuestError(signature, summary, rawText) {
    ...
    if (rawLower.includes('[questerrors]') ||
        rawLower.includes('[**questerrors**]') ||
        rawLower.includes('questerrors]') ||
        rawLower.includes('**questerrors**')) {
        return true;
    }
    if (rawLower.includes('[script error]') ||
    ...
    const hasNativeCrashIndicators = exception.includes('exception_access_violation') ||
    ...
    const hasScriptErrorIndicators = exception.includes('[questerrors]') ||
...
// ============================================================================
export async function markMessageRead(messageId, userId, read) {
    if (read) {
        // Upsert read status
...
export async function getUnreadCount(userId, webhookId) {
    const whereMessage = {
        archivedAt: null
    };
    if (webhookId) {
...
// Star Operations
// ============================================================================
export async function toggleMessageStar(messageId, userId, starred) {
    if (starred) {
        // Upsert star
...
export async function findOrCreateWebhookLabel(name) {
    ...
    const existingLabel = allLabels.find((l) => l.name.toLowerCase() === trimmedName.toLowerCase());
    if (existingLabel) {
...
export async function updateWebhookLabel(labelId, input) {
    const data = {};
    if (input.name !== undefined)
        data.name = input.name.trim();
...
export async function deleteWebhookLabel(labelId) {
    await prisma.webhookMessageLabel.delete({
        where: { id: labelId }
    });
}
export async function setMessageLabels(messageId, labelIds) {
...
async function getOrCreateDefaultLabel(type) {
...
async function applyImmediateLabels(messageId, rawText, webhookLabel) {
    try {
        const labelsToAdd = [];
        const labelNames = [];
        // Detect if this is a quest/script error vs crash using pattern matching
        const isQuestError = detectQuestError(undefined, // No signature needed for pattern detection
        '', // No summary needed
...
async function autoLabelMessage(messageId, findings, signature, webhookLabel, rawText) {
    try {
        ...
        if (!hasCrashTypeLabel) {
            ...
            const isQuestError = detectQuestError(signature, typeof findings?.summary === 'string'
                ? findings.summary
                : '', rawText);
            // Add crash type label (Crash or Script Error)
            const crashLabelId = await getOrCreateDefaultLabel(isQuestError ? 'SCRIPT_ERROR' : 'CRASH');
...
// Message Merging
// ============================================================================
export async function mergeWebhookMessages(messageIds, combinedText) {
...
export async function bulkMessageAction(messageIds, action, userId, options) {
    let success = 0;
    let failed = 0;
...
export async function listInboundWebhookMessagesEnhanced(options) {
    const { page, pageSize, userId, webhookId, status, includeArchived, readStatus, starred, labelIds } = options;
    ...
    let messageIdsToInclude;
    let messageIdsToExclude;
    if (readStatus === 'read') {
 ...
 */
function addMessageToPendingGroup(webhookId, messageId, groupKey, mergeWindowSeconds) {
    const now = new Date();
 ...
 */
function findExistingGroupForWebhook(webhookId) {
 ...
 */
async function processGroupWhenReady(compositeKey) {
 ...
 */
export async function processGroupNow(webhookId, groupKey) {
 ...
 */
export function getPendingMergeGroups() {
    const now = Date.now();
 ...
 */
function scheduleDelayedProcessing(webhookId, mergeWindowSeconds) {
 ...
 */
export function isWebhookProcessingPending(webhookId) {
 ...
 */
export function getPendingProcessingWebhookIds() {
 ...
 */
async function processSpecificGroup(group) {
    ...
    try {
        ...
        if (messages.length === 1) {
            ...
            const discordAction = webhook.actions.find((action) => action.isEnabled && action.type === 'DISCORD_RELAY');
            if (discordAction) {
            ...
            const clawdbotAction = webhook.actions.find((action) => action.isEnabled && action.type === 'CLAWDBOT_RELAY');
            if (clawdbotAction) {
 ...
 */
async function processWebhookMessages(webhookId, isRetry = false) {
 ...
 */
async function markMessagesAsPendingMerge(messageIds) {
 ...
 */
async function sendPendingMergeNotification(webhook, messages) {
 ...
 */
export async function processMessagesAfterManualMerge(mergedMessageId) {
    const message = await prisma.inboundWebhookMessage.findUnique({
 ...
 */
export async function processDismissedMergeMessages(messageIds) {
    for (const messageId of messageIds) {
        const message = await prisma.inboundWebhookMessage.findUnique({
            where: { id: messageId },
            include: {
 ...
 */
function extractCrashFileIdentifier(payload, rawBody) {
 ...
 */
function groupMessagesByCrashFile(messages) {
 ...
 */
function groupMessagesForMerge(messages) {
    const groups = new Map();
    for (const message of messages) {
 ...
 */
async function processAutoMergeGroup(messages, webhook) {
 ...
 */
function extractActualMessageContent(payload, rawBody) {
    if (payload && typeof payload === 'object' && !Array.isArray(payload)) {
        const record = payload;
 ...
 */
function extractTextForSorting(payload, rawBody) {
    if (payload && typeof payload === 'object' && !Array.isArray(payload)) {
        const record = payload;
        if (typeof record.crashReportText === 'string') {
            return record.crashReportText;
