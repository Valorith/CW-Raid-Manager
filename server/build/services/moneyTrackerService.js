 ...
 */
function calculatePlatinumEquivalentInCopper(platinum, gold, silver, copper) {
 ...
 */
export async function fetchServerCurrencyTotals() {
    if (!isEqDbConfigured()) {
  ...
  `;
    const [totals] = await queryEqDb(totalsQuery);
    const row = totals;
    ...
    const totalPlatinumEquivalent = (platinum + platinumBank + platinumCursor) * BigInt(COPPER_PER_PLATINUM) +
 ...
 */
export async function fetchTopCharactersByCurrency(limit) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured. Set EQ_DB_* environment variables.');
    }
    ...
    ${limitClause}
  `;
    const rows = await queryEqDb(topCharactersQuery);
    return rows.map((row) => {
        const totalCopper = calculatePlatinumEquivalentInCopper((row.platinum || 0) + (row.platinum_bank || 0) + (row.platinum_cursor || 0), (row.gold || 0) + (row.gold_bank || 0) + (row.gold_cursor || 0), (row.silver || 0) + (row.silver_bank || 0) + (row.silver_cursor || 0), (row.copper || 0) + (row.copper_bank || 0) + (row.copper_cursor || 0));
 ...
 */
export async function fetchSharedBankTotals() {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured. Set EQ_DB_* environment variables.');
    }
    const totalsQuery = `
 ...
 */
export async function fetchTopSharedBanks(limit) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured. Set EQ_DB_* environment variables.');
    }
    const limitClause = limit && limit > 0 ? `LIMIT ${limit}` : '';
    ...
    ${limitClause}
  `;
    const rows = await queryEqDb(topSharedBanksQuery);
    return rows.map((row) => ({
        id: row.id,
        name: row.name || '',
 ...
 */
export async function fetchGuildBankTotals() {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured. Set EQ_DB_* environment variables.');
    }
 ...
 * Note: Values in data_buckets are stored in copper, so we divide by 1000 to get platinum
 */
export async function fetchTopGuildBanks(limit) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ database is not configured. Set EQ_DB_* environment variables.');
    }
    const limitClause = limit && limit > 0 ? `LIMIT ${limit}` : '';
    ...
    ${limitClause}
  `;
    const rows = await queryEqDb(topGuildBanksQuery);
    // Convert from copper to platinum (divide by 1000)
    return rows.map((row) => ({
        id: row.id,
        name: row.name || '',
 ...
 */
export async function createMoneySnapshot(createdById) {
    ...
    return {
        ...snapshot,
        topCharacters: snapshot.topCharacters,
        topGuildBanks: snapshot.topGuildBanks
    };
}
/**
 * Get all snapshots within a date range
 */
export async function getSnapshotsInRange(startDate, endDate, limit = 365) {
    const where = {};
 ...
 */
export async function getMoneyTrackerSummary() {
 ...
 */
export async function updateSettings(data, updatedById) {
 ...
 */
export async function updateLastSnapshotTime() {
