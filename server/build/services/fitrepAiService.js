...
function parsePrompt(prompt) {
    ...
    if (/early promote|ep\b|#1|top|best|finest|extraordinary/i.test(prompt))
        recommendation = 'Early Promote';
    else if (/must promote|mp\b|outstanding|exceptional|superior/i.test(prompt))
        recommendation = 'Must Promote';
    else if (/progressing|developing|needs improvement|below/i.test(prompt))
    ...
    // Extract rank
    let rank;
    if (/master chief|mcpo|e-?9/i.test(prompt))
        rank = 'E9';
    else if (/senior chief|scpo|e-?8/i.test(prompt))
        rank = 'E8';
    else if (/chief|cpo|e-?7/i.test(prompt))
        rank = 'E7';
...
export function generateSuggestions(request) {
    ...
    const cmdEmploy = fillTemplate(COMMAND_EMPLOYMENT_TEMPLATES[Math.floor(Math.random() * COMMAND_EMPLOYMENT_TEMPLATES.length)], vars);
    const duties = fillTemplate(PRIMARY_DUTIES_TEMPLATES[Math.floor(Math.random() * PRIMARY_DUTIES_TEMPLATES.length)], vars);
