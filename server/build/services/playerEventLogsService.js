...
let zonesCache = { data: null, timestamp: 0 };
 ...
 */
export async function fetchPlayerEventLogs(filters = {}) {
    if (!isEqDbConfigured()) {
        throw new Error('EQ content database is not configured; set EQ_DB_* values to enable player event logs.');
    }
    const { page = 1, pageSize: requestedPageSize = 25, characterId, characterName, eventTypes, zoneId, startDate, endDate, sortBy = 'created_at', sortOrder = 'desc', search: rawSearch } = filters;
    ...
    const [countResult] = await queryEqDb(countQuery, params);
 ...
 */
export async function getPlayerEventLogStats() {
    if (!isEqDbConfigured()) {
        throw new Error('EQ content database is not configured; set EQ_DB_* values to enable player event logs.');
    }
 ...
 */
export async function getEventLogZones() {
    if (!isEqDbConfigured()) {
        return [];
    }
