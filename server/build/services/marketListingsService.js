...
const MARKET_LISTINGS_CACHE_UNAVAILABLE_MESSAGE = 'Bazaar listings cache is unavailable. Apply the market listings migration first.';
const CREATE_MANY_CHUNK_SIZE = 500;
const VALID_ITEM_TYPE_VALUES = new Set(EQEMU_ITEM_TYPE_VALUES);
const VALID_EQUIP_SLOT_IDS = new Set(EQEMU_ITEM_SLOT_IDS);
const SELLER_EVENT_TYPE = 'TRADER_SELL';
let marketListingsSyncPromise = null;
...
function buildEmptyMarketListingsPage(page, pageSize, syncStatus, message, sourceAvailable = false) {
    return {
        listings: [],
        summary: {
            totalListings: 0,
...
function isMissingMarketListingsSource(error) {
...
function isMarketListingCacheSchemaMissing(error) {
...
function logMissingMarketListingsCache(logger) {
    if (marketListingsCacheMissingWarned) {
        return;
    }
    marketListingsCacheMissingWarned = true;
    logger?.warn?.('[MarketListings] MarketListing cache schema is missing expected columns. Apply the market listings migrations.');
}
function buildWhereClause(filters) {
    ...
    if (searchQuery) {
        ...
        conditions.push("(LOWER(COALESCE(ml.itemName, '')) LIKE ? OR LOWER(COALESCE(ml.sellerCharacterName, '')) LIKE ?)");
...
function buildOrderByClause(sortBy, sortOrder, includeAveragePrice) {
    switch (sortBy) {
        case 'price':
...
function mapMarketListingRow(row) {
...
function buildAveragePriceJoinClause(includeAveragePrice) {
    if (!includeAveragePrice) {
        return '';
    }
    return `
...
async function getMarketListingsSyncState() {
...
async function setMarketListingsSyncState(state, db = prisma) {
...
async function fetchEqMarketListings() {
    return queryEqDb(`
      SELECT
...
function buildCacheRecords(rows, syncedAt) {
    return rows.map((row) => {
        const sellerCharacterId = toNumber(row.sellerCharacterId);
        const itemId = toNumber(row.itemId);
        return {
...
async function replaceMarketListingsCache(records, syncedAt) {
    await prisma.$transaction(async (tx) => {
        ...
        for (let index = 0; index < records.length; index += CREATE_MANY_CHUNK_SIZE) {
            ...
            VALUES ${Prisma.join(chunk.map((record) => Prisma.sql `(
                ${record.id},
...
export async function getMarketListingsSyncStatus() {
    const state = await getMarketListingsSyncState();
    return {
        lastRetrievedAt: state.lastRetrievedAt
    };
}
export async function syncMarketListings(options = {}) {
    if (!isEqDbConfigured()) {
        return {
            retrieved: 0,
    ...
    marketListingsSyncPromise = (async () => {
        try {
            ...
            const existingRows = await prisma.marketListing.count().catch((error) => {
            ...
            if (existingRows > 0 && lastRetrievedMs && Date.now() - lastRetrievedMs < minIntervalMs) {
                logger?.debug?.(`[MarketListings] skipped early retrieval; lastRetrievedAt=${status.lastRetrievedAt ?? 'n/a'}`);
                return {
                    retrieved: 0,
            ...
            if (isMissingMarketListingsSource(error)) {
                logger?.warn?.('[MarketListings] EQ trader tables are unavailable. Preserving the existing listings cache.');
                return {
                    retrieved: 0,
                    lastRetrievedAt: (await getMarketListingsSyncStatus()).lastRetrievedAt
                };
...
export async function ensureMarketListingsFresh(options = {}) {
    if (!isEqDbConfigured()) {
        return;
    }
    ...
    const [status, totalRows] = await Promise.all([
        ...
        prisma.marketListing.count().catch((error) => {
            if (isMarketListingCacheSchemaMissing(error)) {
                logMissingMarketListingsCache(logger);
                return 0;
            }
...
export async function getMarketListingsPage(options = {}) {
    const { q = '', itemName, sellerName, itemType, equipSlot, minPrice, maxPrice, minCharges, maxCharges, listedWithinDays, dealsOnly = false, page = 1, pageSize = 25, sortBy = 'listedAt', sortOrder = 'desc' } = options;
    ...
    const runListingsQuery = async (includeAveragePrice) => {
    ...
    try {
        ...
        if (isMarketSaleEventTableMissing(error)) {
            if (dealsOnly) {
                return {
                    ...buildEmptyMarketListingsPage(page, pageSize, syncStatus, 'No listings match the current filters.'),
                    sourceAvailable: true
                };
            }
        ...
        if (isMarketListingCacheSchemaMissing(error)) {
            return buildEmptyMarketListingsPage(page, pageSize, syncStatus, MARKET_LISTINGS_CACHE_UNAVAILABLE_MESSAGE);
        }
        throw error;
    }
}
