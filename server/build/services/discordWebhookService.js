...
export async function isDiscordWebhookEventEnabled(guildId, event) {
    if (!guildId) {
        return false;
    }
    const records = await prisma.guildDiscordWebhook.findMany({
...
function resolveClientBaseUrl() {
    ...
    if (!resolved) {
        console.warn('CLIENT_URL (or equivalent) is not configured; Discord webhooks will omit direct links. Set CLIENT_URL or PUBLIC_CLIENT_URL to your production site.');
        return null;
    }
    if (isLocalHost(resolved)) {
        console.warn(`Resolved client URL "${resolved}" appears to be a localhost address. Configure CLIENT_URL to ensure webhook links point to your production site.`);
    }
    return resolved;
}
