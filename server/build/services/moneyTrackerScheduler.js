...
import { createMoneySnapshot, getSettings, updateLastSnapshotTime } from './moneyTrackerService.js';
 ...
 */
async function executeScheduledSnapshot() {
 ...
 */
async function executeMetallurgySnapshot() {
    if (!isEqDbConfigured()) {
        return;
    }
    try {
 ...
 */
async function executeAutoLinkSharedIps() {
    if (!isEqDbConfigured()) {
        return;
    }
    try {
        ...
        logger.info(`[MoneyTrackerScheduler] Auto-link completed: ${result.created} created, ${result.skipped} skipped, ${result.sharedIpsFound} shared IPs found.`);
    }
    catch (error) {
 ...
 */
export function startMoneyTrackerScheduler(customLogger) {
 ...
 */
export function stopMoneyTrackerScheduler() {
    if (schedulerInterval) {
 ...
 */
export function resetLastRunDate() {
    lastRunDate = null;
}
