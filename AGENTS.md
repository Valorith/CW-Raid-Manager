...
- Services + ports: the API (Fastify) listens on `4000`, the Vite client on `5173`, and a local MariaDB (as a MySQL server) on `127.0.0.1:3306`. The Vite client proxies `/api` and `/assets/icons` to `4000`. Standard dev/build/test commands live in the root and workspace `package.json` scripts and in `README.md` / the "Build, Test, and Development Commands" section above; prefer those.
- The environment `start` script brings up MariaDB, ensures the `cw_raid_manager` DB + `cwraid` user, writes `server/.env` if missing (local dev `DATABASE_URL`/`SESSION_SECRET`), runs `prisma db push`, and then runs `npm run dev`. If services are not up, re-run that flow; `server/.env` is git-ignored local config.
- Fresh-DB schema gotcha: apply the schema with `prisma db push` (which is what `start` does), NOT the `prisma:migrate` + `knex:migrate` sequence from `README.md`. On a brand-new database, running the Prisma migration history in order fails because a Prisma migration (`20260127_add_webhook_dev_mode`) alters `InboundWebhook`, a table created by a Knex migration — the two histories are not linearly ordered. `prisma db push` reconciles the DB directly to `schema.prisma`, which is the source of truth for the generated client the app actually uses. Use the documented migrate commands only when authoring a real schema change.
- Auth in cloud: Google/Discord OAuth is not configured (no credentials), so UI sign-in is unavailable by default. To exercise authenticated flows, seed a user directly via Prisma, then either (a) create a `CliSession` row and call APIs with `Authorization: Bearer nxcli_...` (CLI tokens are scoped to `/api/auth/me`, Test Manager, and Webhook Inbox), or (b) mint a `cwraid_token` cookie: JWT `{ userId, email }` signed with `SESSION_SECRET` (fast-jwt HS256), then sign that value with `@fastify/cookie`'s `sign(value, SESSION_SECRET)`. To enable real OAuth instead, provide `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` (and/or Discord) as secrets and add them to `server/.env`.
- Known lint failure: `npm run lint` fails on `server/src/app.ts` with `import/default` for `@fastify/cookie` (the CJS default export is not resolved by `eslint-plugin-import`). This is pre-existing dependency drift (no committed lockfile, no lint CI); `tsc`/`vue-tsc` builds and the app itself are unaffected.
