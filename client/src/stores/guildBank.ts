...
export const useGuildBankStore = defineStore('guildBank', () => {
  ...
  async function fetchSnapshot(guildId: string, force = false) {
    ...
    try {
      ...
      const characterMap = new Map(
        bankResult.characters.map((c) => [c.name.toLowerCase(), { ...c, isTracked: true }])
      );
      ...
      if (snapshot.value) {
        console.log(
          `[GuildBankStore] Loaded ${snapshot.value.characters.length} characters (merged) for guild ${guildId}`
        );
        console.log(`[GuildBankStore] Loaded ${snapshot.value.items.length} items total.`);

        // Debug: Check for specific character items
        const toggItems = snapshot.value.items.filter((i) =>
          i.characterName.toLowerCase().includes('togg')
        );
        ...
        console.log(
          '[GuildBankStore] Sample characters:',
          snapshot.value.characters.slice(0, 5).map((c) => c.name)
        );
      }
    } catch (err: any) {
  ...
  function openModal(guildId: string, characterName: string, slotId?: number) {
    ...
    if (!snapshot.value || loadedGuildId.value !== guildId) {
      ...
      const found = snapshot.value.characters.find(
        (c) => c.name.toLowerCase() === characterName.toLowerCase()
      );
      if (!found) {
        console.warn(
          `[GuildBankStore] Character ${characterName} not found in loaded snapshot. Force refreshing...`
        );
      ...
      if (snapshot.value && snapshot.value.items) {
        ...
        if (uniqueMatches.length > 0) {
          ...
          const sampleOwners = [
            ...new Set(snapshot.value.items.slice(0, 10).map((i) => i.characterName))
          ];
  ...
  async function openCharacterInventory(characterName: string, contextGuildId: string) {
    ...
    if (!snapshot.value || loadedGuildId.value !== contextGuildId) {
      console.log(
        `[GuildBankStore] Context mismatch (loaded: ${loadedGuildId.value}, needed: ${contextGuildId}), loading snapshot first...`
      );
    ...
    if (snapshot.value && loadedGuildId.value === contextGuildId) {
      ...
      const hasItems = snapshot.value.items.some(
        (i) => i.characterName.toLowerCase() === lowerName
      );

      if (found && hasItems) {
        console.log(
          `[GuildBankStore] Character ${firstName} found in current snapshot with items.`
        );
        openModal(contextGuildId, firstName);
        return;
      } else if (found) {
        console.log(
          `[GuildBankStore] Character ${firstName} found in snapshot but has 0 items. Attempting direct lookup...`
        );
      }
    }
    ...
    console.log(
      `[GuildBankStore] Character ${firstName} not in snapshot, attempting direct lookup...`
    );
    try {
      ...
      const otherItems = snapshot.value.items.filter(
        (i) => i.characterName.toLowerCase() !== lowerName
      );
