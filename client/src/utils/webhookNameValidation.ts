...
export function getWebhookNamePlaceholder(context: 'inbound' | 'outbound' | 'action'): string {
  if (context === 'inbound') {
    return 'e.g., Live Server Reports';
  }
  if (context === 'outbound') {
    return 'e.g., Devin crash telemetry';
  }
  return 'e.g., Live Server Slack #crashes-and-errors';
}

export function getWebhookNameHint(context: 'inbound' | 'outbound' | 'action'): string {
  if (context === 'inbound') {
    return 'Format: {source} {kind} (e.g., "Live Server Reports", "Test Server Crash Telemetry")';
  }
  if (context === 'outbound') {
    return 'Format: {destination} {purpose} (e.g., "Devin crash telemetry", "Clumsy Ops crash relay")';
  }
  return 'Format: {parent webhook label} {destination or job} (e.g., "Live Server Slack #crashes-and-errors", "Live Server Gemini crash review")';
}
